...
#include <SFML/Graphics.hpp>
    ...
    sf::Color ToSFMLColor(b2HexColor color) {
    ...
    sf::Vector2f ToSFMLVec2(const b2Vec2& v, float scale = 30.f) {
        return { v.x * scale, v.y * scale };
    }

    void SFMLDrawPolygon(const b2Vec2* verts, int count, b2HexColor color, void* ctx) {
    ...
    void SFMLDrawSolidPolygon(b2Transform xf, const b2Vec2* verts, int count,
    ...
    void SFMLDrawCircle(b2Vec2 center, float radius, b2HexColor color, void* ctx) {
        auto& win = *static_cast<sf::RenderWindow*>(ctx);
        //float scale = 30.f;
        ...
        circle.setPosition(ToSFMLVec2(center)); //- sf::Vector2f(radius * scale, radius * scale));
        win.draw(circle);
    }

    void SFMLDrawSolidCircle(b2Transform xf, float radius, b2HexColor color, void* ctx) {
        auto& win = *static_cast<sf::RenderWindow*>(ctx);
        b2Vec2 center = xf.p;
        sf::CircleShape circle(radius);
        ...
        circle.setOutlineColor(ToSFMLColor(color));
        circle.setOutlineThickness(2.f);
        circle.setPosition(ToSFMLVec2(center));
        win.draw(circle);
    }

    void SFMLDrawSegment(b2Vec2 p1, b2Vec2 p2, b2HexColor color, void* ctx) {
        auto& win = *static_cast<sf::RenderWindow*>(ctx);
    ...
    void SFMLDrawPoint(b2Vec2 p, float size, b2HexColor color, void* ctx) {
        auto& win = *static_cast<sf::RenderWindow*>(ctx);
        float scale = 30.f;
    ...
    void CreateSFMLDebugDraw(b2DebugDraw& debugDraw, sf::RenderWindow* context) {
        ...
        debugDraw.context = context; // pass SFML render target
    }
